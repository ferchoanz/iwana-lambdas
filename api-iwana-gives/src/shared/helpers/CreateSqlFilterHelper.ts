...
class CreateFilterHelper<T = any> {
    ...
    createFilter(attr: AttributeConfig<T> | string, condition: FilterCondition, operator: FilterOperator, alias = 'i'): void {
                ...
                new Brackets((qb) => {
                    ...
                    values.map((_attr: string, index: number) => {
                        ...
                        if (valueAttr.length > 0) {
                            if (toLower) {
                                qb[where](`LOWER(${alias}.${dbAttribute}) ${operator} LOWER(:${aliasAttr})`, {
                                    [aliasAttr]: valueAttr,
                                });
                            } else {
                                qb[where](`${alias}.${dbAttribute} ${operator} :${aliasAttr}`, {
                                    [aliasAttr]: valueAttr,
                                });
                            }
                        }
    ...
    private vectorSearch(
        ...
        if (_.isString(attrsDB)) {
            searchAtt = `to_tsvector(${alias}.${String(attrsDB)})`;
        }
        ...
        if (_.isObject(attrsDB)) {
            ...
            if (_.isObject(attrsDB[0])) {
                searchAtt = (<AttributeDBConfig[]>attrsDB)
                    .map((attrDB: AttributeDBConfig) => {
                        const coalesce: boolean = _.isUndefined(attrDB?.coalesce) ? false : attrDB.coalesce;
                        const setWeight: boolean | SetWeightRelevance = _.isUndefined(attrDB?.setWeight)
                            ? false
                            : attrDB.setWeight;
    ...
    private iLikeSearch(
        ...
        if (_.isString(attrsDB)) {
            searchAtt = `${alias}.${String(attrsDB)}`;
        }
        ...
        if (this._filter.has(attr)) {
            const valueAttr: string | string[] = this._filter
                .get(attr)
                ?.trim()
                .split(' ')
                .map((_attr: string) => `%${_attr}%`)
                .join(' ');
